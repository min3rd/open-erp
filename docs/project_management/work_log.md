...
- **Rà Soát & Sửa Tài Liệu Sprint 02 (2026-09-18)**:
  - Review toàn bộ 22 tài liệu Sprint-Pack 02 + kiểm tra chéo liên kết/schema/items/test plan; phát hiện 24 nhóm vấn đề: 1 link hỏng, mâu thuẫn bộ 7 Data Scopes (CUSTOM vs NONE), thiếu cơ chế enforce RBAC chức năng, thiếu Reference Entity để kiểm chứng data-scope, quota chưa enforce, xung đột nguồn vai trò `user_tenants.role` vs `user_roles`, state machine Tenant lệch schema, thiếu Break-Glass, thiếu `target_user_id` khi impersonate, thiếu bootstrap Super Admin, thiếu backfill migration, thiếu endpoint/i18n mapping/TC, ràng buộc toàn vẹn DB, guard FE/Mobile, trạng thái Confirmation Gate chưa ký.
  - Tạo **43 item** trong `07_items/`: BUG-49 → BUG-70 (22 bug), TASK-267 → TASK-286 (20 task), **FEAT-17** Reference Entity `core_sample_records` (High) + 4 sub-task.
  - Sửa đồng bộ tài liệu BA/PM: `00_READING_GUIDE` (trạng thái gate, 11 bảng, quy ước mã tài liệu), `CONF-01` (trạng thái chờ duyệt + **Phụ lục rà soát** + FEAT-17 In-Scope), `sprint_plan` (FEAT-17, mã code thống nhất, claim `groups`), `ANL-01` (EXPIRED, banner chuẩn hóa, nguyên tắc bảo mật 6-7), `ANL-02` (bỏ CUSTOM, `assignee_id`, Anti-Modal wording, BR-RBAC-06/07), `test_plan` (openerp_test + Flyway V2 + TC-BE-13→20 + dùng `core_sample_records`), 7 file FEAT tham chiếu item, đổi tên `09_review/sprint_review.md` + bổ sung Deferred/Retrospective.
  - Sửa đồng bộ tài liệu Kiến trúc: `SOL-01` (issuer, `groups`, bootstrap, target_user_id + guard exit, secret-export guard, DB role, health enum), `SOL-02` (chốt Redis Pub/Sub, `assignee_id`, CREATE rules, BFS, `@RequirePermission`, Hibernate Filter), DB (`TenantStatus` + `is_locked` mapping, partial unique, composite FK chéo tenant, bỏ `custom_conditions`, thêm `core_sample_records`, kế hoạch Flyway V2.0.0/V2.0.1), API (fix link, chuẩn hóa mã `PLATFORM_*`, 201, thêm 6 nhóm endpoint còn thiếu, bảng i18n đầy đủ, health DEGRADED/UNKNOWN, claim `groups`), UI (legend `DEPT+`, banner chuẩn hóa, mục Routes & Guard, menu mobile, i18n).
  - **Trạng thái Sprint 02 (tại thời điểm rà soát)**: tài liệu Bước 1-7 đã hoàn tất và đồng bộ; khi đó đang chờ khách hàng ký Confirmation Gate — **nay đã được phê duyệt ngày 2026-09-18 (xem block "PHÊ DUYỆT CONFIRMATION GATE SPRINT 02" cuối file).**
  - **Bổ sung thiết kế Quản lý đa chi nhánh (2026-09-18)**: chốt `BRANCH` scope = hợp của Chi nhánh thành viên và Chi nhánh được quản lý; thêm bảng `user_branch_assignments` + 1 primary branch/user; auto-sync membership từ `departments.manager_user_id`; cập nhật ANL-02 (BR-RBAC-08→11), SOL-02 (context 4 trường branch + mục 4.1 + CREATE default), DES-DB (§3.4 bảng mới; tổng 12 bảng cấu trúc + 1 Reference Entity), DES-API (§4.4 CRUD branch-assignments + i18n), DES-UI (`BranchAssignmentDrawer` + badge), test_plan (User 8/9 + TC-BE-21→23), CONF-01 Phụ lục mục 7; tạo **BUG-71** (Done) + **TASK-287 → TASK-290**.
  - **Bổ sung thiết kế lưu trữ Audit Log (2026-09-18)**: hash chain SHA-256 + 4 lớp bất biến, partition tháng + retention 24 tháng + cold archive (MongoDB/S3, sprint sau), audit dùng chung scope PLATFORM/TENANT, coverage matrix 20+ action, write-path fail-closed; cập nhật SOL-01 §3, DES-DB §2.4, DES-API §3.7, DES-UI, test_plan TC-BE-24→27, FEAT-12 AC5, CONF-01 Phụ lục mục 8; tạo BUG-72 (Done) + TASK-291→293.
  - **Bổ sung vòng đời Super Admin & CLI (2026-09-18)**: 3 con đường cấp quyền (bootstrap/API/CLI), không tự đăng ký, lifecycle INVITED→ACTIVE→DISABLED→REVOKED, guard self-disable + last-admin, CLI offline/remote có audit; cập nhật SOL-01 §1.2, DES-DB, DES-API §3.12, DES-UI, test_plan TC-BE-28→32, CONF-01 Phụ lục mục 9; tạo FEAT-18 + BUG-73 (Done) + TASK-294→296.

- **PHÊ DUYỆT CONFIRMATION GATE SPRINT 02 (2026-09-18)**:
  - Khách hàng xác nhận phạm vi Sprint 02 và **toàn bộ 9 mục Phụ lục rà soát** (FEAT-17 Reference Entity, Break-Glass APIs, Quota Enforcement, Role source migration, Tenant EXPIRED, Scope set 7 giá trị, Multi-Branch Manager, Audit Log Storage, Super Admin Lifecycle & CLI).
