...
* **Nhiệm vụ 1: Thiết kế Lõi Hành động và Máy trạng thái (Finite State Machine - FSM) & Đánh giá Gộp luồng (Join Sync)**
  ...
  - **Xử lý hội tụ nhánh song song (Join Sync):** Khi một bước duyệt hoàn thành, kiểm tra xem bước tiếp theo có phải là nút `JOIN` hay không. Nếu có, thực hiện quét bảng `workflow_approvers` đối với tất cả các nhánh song song đồng cấp. Chỉ khi đạt điều kiện cấu hình gộp luồng (`join_rules` - ví dụ: tất cả các nhánh đều `APPROVED`), hệ thống mới kích hoạt bước tuần tự tiếp theo và cập nhật `current_step_ids` của instance.
