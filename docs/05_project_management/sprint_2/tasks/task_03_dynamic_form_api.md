...
* **TypeORM Entity (Cấu trúc DB Schema):**
  - [DynamicForm entity](../../../../open-erp-services/src/core/dynamic-form/entities/dynamic-form.entity.ts) - Bảng `dynamic_forms` với các cột: `id`, `tenant_id`, `form_key`, `name`, `description`, `version`, `is_latest`, `fields` (JSONB), `created_at`, `updated_at`. Ràng buộc unique `(tenant_id, form_key, version)` được đảm bảo qua logic nghiệp vụ.

* **Core Service Logic & Cập nhật nâng cao (TSK-2.17):**
  - [DynamicFormService](../../../../open-erp-services/src/core/dynamic-form/dynamic-form.service.ts) cung cấp các hàm:
    ...
    - `validateFieldSchemas` (private): Kiểm tra tính hợp lệ của cấu hình trường JSON do admin gửi lên. **Cập nhật nâng cao:** Hỗ trợ kiểu linh kiện đặc biệt `GRID` (bảng/lưới Excel). Thực hiện kiểm tra tính hợp lệ của các `columns` con bên trong grid (kiểu cột, tên cột trùng lặp, options của SELECT column).
    - `runValidation` (private): Validate dữ liệu gửi lên. **Cập nhật nâng cao:** Hỗ trợ lặp qua dữ liệu mảng GRID và thực hiện validate động (nested cell validation) từng thuộc tính cột con theo cấu hình schema cột, lưu vết lỗi theo định dạng `proposalItems[i].itemName`.
...
* **REST APIs** (bảo vệ bởi `JwtAuthGuard`):
  ...
  - `POST /api/v1/dynamic-forms/:id/validate` — Validate dữ liệu nhập theo schema (kể cả validate lưới GRID).
  - Thiết lập tại [DynamicFormController](../../../../open-erp-services/src/features/dynamic-form/dynamic-form.controller.ts).

* **Kiểm thử tự động (Unit Tests):**
  - [dynamic-form.service.spec.ts](../../../../open-erp-services/src/core/dynamic-form/dynamic-form.service.spec.ts) — **26 test cases** (Đã bổ sung 5 test cases chuyên biệt cho GRID schema & data validation), tất cả PASS.
  - [dynamic-form.controller.spec.ts](../../../../open-erp-services/src/features/dynamic-form/dynamic-form.controller.spec.ts) — 8 test cases bao phủ toàn bộ 4 endpoints.
  - Tổng cộng **34 test cases**, tất cả PASS, build không có lỗi TypeScript.
