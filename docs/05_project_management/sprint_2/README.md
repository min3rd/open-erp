...
| **TSK-2.21** | API getLatestByKey cho DynamicForm | Thêm phương thức `getLatestByKey(key)` trong DynamicFormService và endpoint `GET /dynamic-forms/key/:key` trả về bản form mới nhất. | [x] Done | BE Engineers | [task_21_get_latest_by_key.md](./tasks/task_21_get_latest_by_key.md) |
...
| **TSK-2.20** | Thư viện Canvas vẽ đồ họa dùng chung | Xây dựng thư viện `@open-erp/shared-ui/canvas` để vẽ nodes, edges, hiển thị dữ liệu trên SVG canvas, hỗ trợ pan/zoom, auto-layout và undo/redo cho Workflow Designer. | [x] Done | FE Web Engineers | [task_20_canvas_library.md](./tasks/task_20_canvas_library.md) |
| **BUG-2.1** | Lỗi FORWARD trong WorkflowApproverStatus | Bổ sung trạng thái `FORWARDED` vào enum `WorkflowApproverStatus` và cập nhật service để xử lý hành động chuyển tiếp task (forward task). | [x] Done | BE Engineers | [bug_01_workflow_forward.md](./bugs/bug_01_workflow_forward.md) |
| **TSK-2.10** | Giao diện thiết lập Form động nâng cao | Giao diện Web kéo thả và dùng nút điều hướng thiết kế form động, thiết lập layout đa thiết bị, điều kiện logic và liên kết API. Sử dụng TSK-2.18 & TSK-2.19. | [x] Done | FE Web Engineers | [task_10_dynamic_form_builder_ui.md](./tasks/task_10_dynamic_form_builder_ui.md) |
| **TSK-2.16** | Giao diện thiết kế Workflow nâng cao | Giao diện Web Canvas thiết kế sơ đồ quy trình rẽ nhánh, cấu hình node, hành động/kết quả và hỗ trợ tự động sắp xếp node. Sử dụng TSK-2.19 & TSK-2.20. Infra đã sẵn sàng (Canvas, DnD, Auto-layout). Cần triển khai Node Palette, Configuration Panel, Conditions, JSON Parser. | [ ] Todo | FE Web Engineers | [task_16_workflow_designer_ui.md](./tasks/task_16_workflow_designer_ui.md) |
