...
#### 2.2 Luồng phân tích Tenant ID từ Subdomain
* Khi Client gọi API từ `gotech.open-erp.9ms.io.vn`, Ingress/API Gateway hoặc Middleware trong NestJS backend (`open-erp-services`) tự động phân tích tên miền để trích xuất subdomain `gotech`.
* Backend truy vấn bảng `tenants` để tìm `tenant_id` tương ứng và thiết lập biến ngữ cảnh phiên kết nối RLS PostgreSQL: `SET LOCAL app.current_tenant_id = 'uuid';`.
