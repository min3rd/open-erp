...
let ComponentShowcaseComponent = class ComponentShowcaseComponent {
    ...
    interactiveIconSize = signal(32);
    // Status Badge & KPI Controls
    interactiveStatusType = signal("ACTIVE");
    interactiveKpiTitle = signal("Tổng Doanh thu");
    interactiveKpiValue = signal("485.900.000 ₫");
    // Statistic Controls
    interactiveStatBordered = signal(true);
    // Carousel Controls
    interactiveCarouselAutoplay = signal(true);
    interactiveCarouselDots = signal(true);
    // Empty State Controls
    interactiveEmptyType = signal("not-found");
    // OTP Controls
    interactiveOtpLength = signal(6);
    // Watermark Controls
    interactiveWatermarkOpacity = signal(0.14);
    interactiveWatermarkRotate = signal(-22);
    // Speed Dial Controls
    interactiveSpeedDialDir = signal("up");
    // Sidebar Controls
    interactiveSidebarMode = signal("fixed");
    // Popover Controls
    interactivePopoverPlacement = signal("bottom");
    // Pagination Controls
    interactivePaginationVariant = signal("default");
    interactivePaginationTotal = signal(145);
    // Calendar Controls
    interactiveCalendarCompact = signal(false);
    ...
    sampleAccordionItems = [
        {
            ...
            content: "Hệ thống tự động phát hành hóa đơn khi đơn hàng được đánh dấu hoàn tất và chuyển trực tiếp sang cơ quan thuế.",
            icon: "file-text",
            badge: "Mới",
            expanded: true,
        },
        {
            ...
            content: "Thiết lập ngưỡng tín dụng theo từng phân hạng khách hàng (VIP: 1 tỷ, Tiêu chuẩn: 300 triệu).",
            icon: "shield",
        },
        {
            id: "acc-3",
            ...
            content: "Dữ liệu được backup tự động mỗi ngày vào 02:00 sáng và lưu trữ mã hóa 30 ngày gần nhất.",
            icon: "lock",
        },
    ];
    sampleTimelineItems = [
    ...
    activeComponent = computed(() => {
        return (this.allDocs.find((d) => d.id === this.selectedComponentId()) ||
            this.allDocs[0]);
    });
    selectComponent(id) {
