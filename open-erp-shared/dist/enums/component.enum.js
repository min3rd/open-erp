export var ButtonVariant;
(function (ButtonVariant) {
    ButtonVariant["PRIMARY"] = "primary";
    ButtonVariant["SECONDARY"] = "secondary";
    ButtonVariant["OUTLINE"] = "outline";
    ButtonVariant["DANGER"] = "danger";
    ButtonVariant["GHOST"] = "ghost";
    ButtonVariant["SUCCESS"] = "success";
})(ButtonVariant || (ButtonVariant = {}));
export var ButtonSize;
(function (ButtonSize) {
    ButtonSize["SM"] = "sm";
    ButtonSize["MD"] = "md";
    ButtonSize["LG"] = "lg";
})(ButtonSize || (ButtonSize = {}));
export var BadgeStatus;
(function (BadgeStatus) {
    BadgeStatus["COMPLETED"] = "COMPLETED";
    BadgeStatus["SUCCESS"] = "SUCCESS";
    BadgeStatus["ACTIVE"] = "ACTIVE";
    BadgeStatus["PENDING"] = "PENDING";
    BadgeStatus["WARNING"] = "WARNING";
    BadgeStatus["PROCESSING"] = "PROCESSING";
    BadgeStatus["IN_PROGRESS"] = "IN_PROGRESS";
    BadgeStatus["CANCELLED"] = "CANCELLED";
    BadgeStatus["FAILED"] = "FAILED";
    BadgeStatus["DANGER"] = "DANGER";
    BadgeStatus["INACTIVE"] = "INACTIVE";
    BadgeStatus["DRAFT"] = "DRAFT";
    BadgeStatus["INFO"] = "INFO";
})(BadgeStatus || (BadgeStatus = {}));
export var BadgeVariant;
(function (BadgeVariant) {
    BadgeVariant["SOLID"] = "solid";
    BadgeVariant["SUBTLE"] = "subtle";
    BadgeVariant["OUTLINE"] = "outline";
    BadgeVariant["DOT"] = "dot";
})(BadgeVariant || (BadgeVariant = {}));
export var BadgeColor;
(function (BadgeColor) {
    BadgeColor["PRIMARY"] = "primary";
    BadgeColor["SUCCESS"] = "success";
    BadgeColor["WARNING"] = "warning";
    BadgeColor["DANGER"] = "danger";
    BadgeColor["INFO"] = "info";
    BadgeColor["NEUTRAL"] = "neutral";
})(BadgeColor || (BadgeColor = {}));
export var TagVariant;
(function (TagVariant) {
    TagVariant["SOLID"] = "solid";
    TagVariant["SUBTLE"] = "subtle";
    TagVariant["OUTLINE"] = "outline";
})(TagVariant || (TagVariant = {}));
export var TagColor;
(function (TagColor) {
    TagColor["PRIMARY"] = "primary";
    TagColor["SECONDARY"] = "secondary";
    TagColor["SUCCESS"] = "success";
    TagColor["WARNING"] = "warning";
    TagColor["DANGER"] = "danger";
    TagColor["INFO"] = "info";
    TagColor["NEUTRAL"] = "neutral";
    TagColor["PURPLE"] = "purple";
    TagColor["PINK"] = "pink";
})(TagColor || (TagColor = {}));
export var AvatarSize;
(function (AvatarSize) {
    AvatarSize["XS"] = "xs";
    AvatarSize["SM"] = "sm";
    AvatarSize["MD"] = "md";
    AvatarSize["LG"] = "lg";
    AvatarSize["XL"] = "xl";
})(AvatarSize || (AvatarSize = {}));
export var AvatarShape;
(function (AvatarShape) {
    AvatarShape["CIRCLE"] = "circle";
    AvatarShape["ROUNDED"] = "rounded";
    AvatarShape["SQUARE"] = "square";
})(AvatarShape || (AvatarShape = {}));
export var KpiTrendDirection;
(function (KpiTrendDirection) {
    KpiTrendDirection["UP"] = "up";
    KpiTrendDirection["DOWN"] = "down";
    KpiTrendDirection["NEUTRAL"] = "neutral";
})(KpiTrendDirection || (KpiTrendDirection = {}));
export var EmptyStateType;
(function (EmptyStateType) {
    EmptyStateType["NO_DATA"] = "no-data";
    EmptyStateType["NOT_FOUND"] = "not-found";
    EmptyStateType["ERROR"] = "error";
    EmptyStateType["SUCCESS"] = "success";
    EmptyStateType["MAINTENANCE"] = "maintenance";
})(EmptyStateType || (EmptyStateType = {}));
export var InputSize;
(function (InputSize) {
    InputSize["SM"] = "sm";
    InputSize["MD"] = "md";
    InputSize["LG"] = "lg";
})(InputSize || (InputSize = {}));
export var ValidationStatus;
(function (ValidationStatus) {
    ValidationStatus["NONE"] = "none";
    ValidationStatus["VALID"] = "valid";
    ValidationStatus["INVALID"] = "invalid";
    ValidationStatus["WARNING"] = "warning";
})(ValidationStatus || (ValidationStatus = {}));
export var StepperOrientation;
(function (StepperOrientation) {
    StepperOrientation["HORIZONTAL"] = "horizontal";
    StepperOrientation["VERTICAL"] = "vertical";
})(StepperOrientation || (StepperOrientation = {}));
export var StepperStepStatus;
(function (StepperStepStatus) {
    StepperStepStatus["PENDING"] = "pending";
    StepperStepStatus["CURRENT"] = "current";
    StepperStepStatus["COMPLETED"] = "completed";
})(StepperStepStatus || (StepperStepStatus = {}));
export var RatingSize;
(function (RatingSize) {
    RatingSize["SM"] = "sm";
    RatingSize["MD"] = "md";
    RatingSize["LG"] = "lg";
})(RatingSize || (RatingSize = {}));
export var TypographyVariant;
(function (TypographyVariant) {
    TypographyVariant["H1"] = "h1";
    TypographyVariant["H2"] = "h2";
    TypographyVariant["H3"] = "h3";
    TypographyVariant["H4"] = "h4";
    TypographyVariant["H5"] = "h5";
    TypographyVariant["H6"] = "h6";
    TypographyVariant["LEAD"] = "lead";
    TypographyVariant["BODY"] = "body";
    TypographyVariant["SMALL"] = "small";
    TypographyVariant["MUTED"] = "muted";
    TypographyVariant["CODE"] = "code";
})(TypographyVariant || (TypographyVariant = {}));
export var DividerOrientation;
(function (DividerOrientation) {
    DividerOrientation["HORIZONTAL"] = "horizontal";
    DividerOrientation["VERTICAL"] = "vertical";
})(DividerOrientation || (DividerOrientation = {}));
export var SpinnerSize;
(function (SpinnerSize) {
    SpinnerSize["XS"] = "xs";
    SpinnerSize["SM"] = "sm";
    SpinnerSize["MD"] = "md";
    SpinnerSize["LG"] = "lg";
    SpinnerSize["XL"] = "xl";
})(SpinnerSize || (SpinnerSize = {}));
export var SpinnerVariant;
(function (SpinnerVariant) {
    SpinnerVariant["SPIN"] = "spin";
    SpinnerVariant["DOTS"] = "dots";
    SpinnerVariant["PULSE"] = "pulse";
})(SpinnerVariant || (SpinnerVariant = {}));
export var NavbarPosition;
(function (NavbarPosition) {
    NavbarPosition["STATIC"] = "static";
    NavbarPosition["STICKY"] = "sticky";
    NavbarPosition["FIXED"] = "fixed";
})(NavbarPosition || (NavbarPosition = {}));
export var SidebarMode;
(function (SidebarMode) {
    SidebarMode["FIXED"] = "fixed";
    SidebarMode["MINI"] = "mini";
    SidebarMode["OVERLAY"] = "overlay";
})(SidebarMode || (SidebarMode = {}));
export var BreadcrumbSeparator;
(function (BreadcrumbSeparator) {
    BreadcrumbSeparator["SLASH"] = "slash";
    BreadcrumbSeparator["CHEVRON"] = "chevron";
    BreadcrumbSeparator["ARROW"] = "arrow";
    BreadcrumbSeparator["DOT"] = "dot";
})(BreadcrumbSeparator || (BreadcrumbSeparator = {}));
export var PaginationVariant;
(function (PaginationVariant) {
    PaginationVariant["FULL"] = "full";
    PaginationVariant["SIMPLE"] = "simple";
    PaginationVariant["COMPACT"] = "compact";
})(PaginationVariant || (PaginationVariant = {}));
export var TabsVariant;
(function (TabsVariant) {
    TabsVariant["LINE"] = "line";
    TabsVariant["PILLS"] = "pills";
    TabsVariant["ENCLOSED"] = "enclosed";
    TabsVariant["SEGMENTED"] = "segmented";
})(TabsVariant || (TabsVariant = {}));
export var TabsOrientation;
(function (TabsOrientation) {
    TabsOrientation["HORIZONTAL"] = "horizontal";
    TabsOrientation["VERTICAL"] = "vertical";
})(TabsOrientation || (TabsOrientation = {}));
export var DropdownPlacement;
(function (DropdownPlacement) {
    DropdownPlacement["BOTTOM_START"] = "bottom-start";
    DropdownPlacement["BOTTOM_END"] = "bottom-end";
    DropdownPlacement["TOP_START"] = "top-start";
    DropdownPlacement["TOP_END"] = "top-end";
    DropdownPlacement["LEFT"] = "left";
    DropdownPlacement["RIGHT"] = "right";
})(DropdownPlacement || (DropdownPlacement = {}));
export var SpeedDialDirection;
(function (SpeedDialDirection) {
    SpeedDialDirection["UP"] = "up";
    SpeedDialDirection["DOWN"] = "down";
    SpeedDialDirection["LEFT"] = "left";
    SpeedDialDirection["RIGHT"] = "right";
})(SpeedDialDirection || (SpeedDialDirection = {}));
export var SpeedDialPosition;
(function (SpeedDialPosition) {
    SpeedDialPosition["BOTTOM_RIGHT"] = "bottom-right";
    SpeedDialPosition["BOTTOM_LEFT"] = "bottom-left";
    SpeedDialPosition["TOP_RIGHT"] = "top-right";
    SpeedDialPosition["TOP_LEFT"] = "top-left";
})(SpeedDialPosition || (SpeedDialPosition = {}));
export var BackToTopShape;
(function (BackToTopShape) {
    BackToTopShape["CIRCLE"] = "circle";
    BackToTopShape["ROUNDED"] = "rounded";
    BackToTopShape["PILL"] = "pill";
})(BackToTopShape || (BackToTopShape = {}));
//# sourceMappingURL=component.enum.js.map