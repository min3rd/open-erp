...
export class BadgeComponent {
  ...
  readonly corner = input<BadgeCorner | 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | undefined>(undefined);
  readonly variant = input<BadgeVariant | 'solid' | 'subtle' | 'outline' | 'dot'>(BadgeVariant.SUBTLE);
  readonly color = input<BadgeColor | 'primary' | 'success' | 'warning' | 'danger' | 'info' | 'neutral'>(BadgeColor.PRIMARY);
  ...
  readonly displayCount = computed(() => {
    const c = this.count();
    if (c !== undefined) {
      if (c <= 0 && !this.showZero()) return undefined;
