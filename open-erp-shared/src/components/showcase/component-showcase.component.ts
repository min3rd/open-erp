...
import {
  RadioGroupComponent,
  RadioOption,
} from "../form/radio-group/radio-group.component";
...
import {
  ...
  DateRange,
} from "../form/date-range-picker/date-range-picker.component";
...
import {
  FileUploadComponent,
  UploadedFile,
} from "../form/file-upload/file-upload.component";
import {
  NavbarComponent,
  NavbarItem,
...
import {
  BreadcrumbComponent,
  BreadcrumbItem,
} from "../navigation/breadcrumb/breadcrumb.component";
...
import {
  ...
  DropdownMenuItem,
} from "../navigation/dropdown-menu/dropdown-menu.component";
import {
  BottomNavComponent,
  BottomNavItem,
} from "../navigation/bottom-nav/bottom-nav.component";
import {
  AnchorComponent,
  AnchorItem,
} from "../navigation/anchor/anchor.component";
import { BackToTopComponent } from "../navigation/back-to-top/back-to-top.component";
import {
  SpeedDialComponent,
  SpeedDialAction,
} from "../navigation/speed-dial/speed-dial.component";
import {
  SegmentedControlComponent,
  SegmentedControlOption,
} from "../navigation/segmented-control/segmented-control.component";
...
import {
  ...
  AvatarGroupUser,
} from "../avatar/avatar-group.component";
import {
  AccordionComponent,
  AccordionItem,
...
import {
  ...
  DescriptionItem,
} from "../descriptions/descriptions.component";
...
import { CopyButtonComponent } from "../copy-to-clipboard/copy-button.component";
...
export class ComponentShowcaseComponent {
  ...
  interactiveVariant = signal<
    ...
    | "success"
  >("primary");
  interactiveBadgeVariant = signal<
    BadgeVariant | "solid" | "subtle" | "outline" | "dot"
  >("subtle");
  interactiveBadgeColor = signal<
    | BadgeColor
    | "primary"
    | "success"
    | "warning"
    ...
    | "neutral"
  >("primary");
  interactiveTagVariant = signal<TagVariant | "solid" | "subtle" | "outline">(
    "subtle",
  );
  interactiveTagColor = signal<
    | TagColor
    | "primary"
    | "secondary"
    | "success"
    ...
    | "pink"
  >("purple");
  interactiveTypographyVariant = signal<
    | TypographyVariant
    | "h1"
    | "h2"
    | "h3"
    ...
    | "code"
  >("h2");
  interactiveSpinnerVariant = signal<
    SpinnerVariant | "spin" | "dots" | "pulse"
  >("spin");
  interactiveSpinnerSize = signal<
    SpinnerSize | "xs" | "sm" | "md" | "lg" | "xl"
  >("md");
  interactiveStatus = signal<BadgeStatus | string>("COMPLETED");
  interactiveValidationStatus = signal<
    ValidationStatus | "none" | "valid" | "invalid" | "warning"
  >("none");
  interactiveShape = signal<AvatarShape | "circle" | "rounded" | "square">(
    "rounded",
  );
  interactiveSkeletonShape = signal<"rect" | "circle" | "rounded" | "pill">(
    "rounded",
  );
  interactiveTrend = signal<KpiTrendDirection | "up" | "down" | "neutral">(
    "up",
  );
  ...
  interactiveCardVariant = signal<"elevated" | "outlined" | "filled" | "ghost">(
    "elevated",
  );
  interactiveCardPadded = signal<boolean>(true);
  interactiveAvatarGroupMax = signal<number>(4);
  interactiveBadgeCorner = signal<
  ...
  interactiveImageRounded = signal<"none" | "sm" | "md" | "lg" | "xl" | "full">(
    "lg",
  );
  interactiveImagePreview = signal<boolean>(true);
  interactiveTooltipPlacement = signal<"top" | "bottom" | "left" | "right">(
    "top",
  );
  interactiveBreadcrumbSep = signal<"chevron" | "slash" | "arrow" | "dot">(
    "chevron",
  );
  interactiveTabsVar = signal<"line" | "pills" | "enclosed" | "segmented">(
    "pills",
  );
  ...
  interactiveEmptyType = signal<
    EmptyStateType | "no-data" | "not-found" | "error" | "maintenance"
  >("not-found");
  ...
  // Speed Dial Controls
  interactiveSpeedDialDir = signal<
    SpeedDialDirection | "up" | "down" | "left" | "right"
  >("up");

  // Sidebar Controls
  interactiveSidebarMode = signal<SidebarMode | "fixed" | "mini" | "overlay">(
    "fixed",
  );

  // Popover Controls
  interactivePopoverPlacement = signal<
  ...
  interactiveAlertVariant = signal<
  ...
  interactiveProgressStatus = signal<
    "normal" | "success" | "warning" | "error" | "active"
  >("active");
  interactiveResultStatus = signal<
    "403" | "404" | "500" | "success" | "error" | "warning" | "info"
  >("404");
  interactiveWatermarkText = signal<string>("OPEN ERP 2026");

  // Overlay & Popup Signals
  showModal = signal<boolean>(false);
  interactiveModalSize = signal<"xs" | "sm" | "md" | "lg" | "xl" | "full">(
    "md",
  );
  showDrawer = signal<boolean>(false);
  interactiveDrawerPlacement = signal<"left" | "right" | "top" | "bottom">(
    "right",
  );
  interactiveDrawerSize = signal<"sm" | "md" | "lg" | "xl" | "full">("md");
  showLightbox = signal<boolean>(false);
  selectedPhotoIndex = signal<number>(0);
  interactiveTransitionType = signal<
  ...
  sampleSelectOptions: SelectOption[] = [
    ...
    },
    {
      ...
      value: "finance",
      icon: "dollar-sign",
      description: "Quản lý thu chi và ngân sách",
    },
    {
      label: "Phòng Kinh doanh & Marketing",
      value: "sales",
      icon: "shopping-bag",
      description: "Phát triển khách hàng",
    },
    {
  ...
  sampleAutocompleteItems = [
    ...
    {
      label: "Kỹ sư Phần mềm Cao cấp (Senior Engineer)",
      value: "sr_eng",
      category: "Engineering",
    },
    {
      label: "Chuyên viên Phân tích Nghiệp vụ (BA)",
      value: "ba",
      category: "Product",
    },
  ...
  sampleTableData = [
    ...
    {
      code: "ORD-2026-002",
      customer: "Công ty Viettel Telecom",
      category: "Viễn thông",
      total: "820.000.000",
      status: "Đang xử lý",
    },
  ...
  sampleAccordionItems: AccordionItem[] = [
    {
      id: "acc-1",
      title: "Quy trình xuất hóa đơn điện tử tự động",
      subtitle: "Tích hợp phân hệ kế toán và hóa đơn theo Thông tư 78",
      content:
        "Hệ thống tự động phát hành hóa đơn khi đơn hàng được đánh dấu hoàn tất và chuyển trực tiếp sang cơ quan thuế.",
      icon: "file-text",
      badge: "Mới",
      expanded: true,
    },
    {
      id: "acc-2",
      title: "Thiết lập cảnh báo hạn mức công nợ khách hàng",
      subtitle: "Tự động khóa tạo đơn mới khi vượt hạn mức",
      content:
        "Thiết lập ngưỡng tín dụng theo từng phân hạng khách hàng (VIP: 1 tỷ, Tiêu chuẩn: 300 triệu).",
      icon: "shield",
    },
    {
      id: "acc-3",
      title: "Chính sách bảo mật dữ liệu & Sao lưu",
      subtitle: "Mã hóa AES-256 và lưu trữ đám mây đa vùng",
      content:
        "Dữ liệu được backup tự động mỗi ngày vào 02:00 sáng và lưu trữ mã hóa 30 ngày gần nhất.",
      icon: "lock",
    },
  ];
  ...
  sampleTimelineItems: TimelineItem[] = [
    {
      ...
      description: "Nhân viên kinh doanh tạo báo giá số #BG-2026-89.",
      timestamp: "08:30 18/08/2026",
      color: "primary",
      icon: "file-plus",
      tag: "Kinh doanh",
    },
    {
      id: "tl-2",
      title: "Khách hàng ký hợp đồng điện tử",
      description: "Xác thực chữ ký số qua VNPT-CA thành công.",
      timestamp: "10:45 18/08/2026",
      color: "success",
      icon: "check-circle",
      tag: "Pháp lý",
    },
    {
      id: "tl-3",
      title: "Xuất kho và bàn giao đơn vị vận chuyển",
      description: "Đơn hàng đã bàn giao cho Viettel Post mã vận đơn #VP89218.",
      timestamp: "14:20 18/08/2026",
      color: "warning",
      icon: "truck",
      tag: "Vận hành",
    },
    {
      id: "tl-4",
      title: "Giao hàng thành công & Hoàn tất thanh toán",
      description: "Khách hàng đã nhận đủ chứng từ và hóa đơn VAT.",
      timestamp: "16:50 18/08/2026",
      color: "success",
      icon: "award",
      tag: "Hoàn tất",
    },
      ...
      children: [
        {
          ...
          children: [
            {
              id: "team-fe",
              label: "Frontend Team (Angular 22 / Tailwind CSS)",
              icon: "layout",
            },
            {
              id: "team-be",
              label: "Backend Team (.NET Core / Microservices)",
              icon: "server",
            },
            {
              id: "team-qa",
              label: "QA & Automation Testing Team",
              icon: "check-square",
            },
          ],
        },
        {
          ...
          children: [
            {
              id: "team-enterprise",
              label: "Đội ngũ Khách hàng Doanh nghiệp (B2B)",
            },
  ...
  sampleCarouselSlides: CarouselSlide[] = [
    {
      ...
      subtitle:
        "Tích hợp hóa đơn điện tử tự động, đối soát công nợ thời gian thực và báo cáo lưu chuyển tiền tệ.",
    ...
    },
    {
      ...
      subtitle:
        "Tối ưu hóa vị trí lưu kho, mã vạch QR code tự động và dự báo nhu cầu nhập hàng bằng AI.",
      tag: "Tối ưu Vận hành",
      imageUrl:
    ...
    },
    {
      ...
      subtitle:
        "Biểu đồ KPI trực quan hóa dữ liệu kinh doanh đa chiều, hỗ trợ ra quyết định tức thì.",
  ...
  sampleDescriptionItems: DescriptionItem[] = [
    {
      ...
      value: "Công ty Cổ phần Tập đoàn Open-ERP Việt Nam",
    },
    { label: "Mã số Doanh nghiệp / MST", value: "0109928194" },
    { label: "Người đại diện pháp luật", value: "Nguyễn Văn Minh (CEO)" },
    {
      label: "Gói dịch vụ bản quyền",
      value: "Enterprise Ultimate Cloud",
      badge: "Active VIP",
    },
    ...
    {
      label: "Khu vực máy chủ Data Center",
      value: "VNPT IDC Hòa Lạc (Tier III+)",
    },
  ];
  ...
  sampleCalendarEvents: CalendarEvent[] = [
    {
      date: "2026-08-18",
      title: "Họp giao ban Quý III",
      color: "bg-indigo-600",
    },
    {
      date: "2026-08-20",
      title: "Chốt kỳ tính lương nhân viên",
      color: "bg-emerald-600",
    },
    {
      date: "2026-08-25",
      title: "Quyết toán thuế doanh nghiệp",
      color: "bg-amber-600",
    },
  ];
  ...
  formTextareaVal = signal<string>(
    "Giao hàng vào giờ hành chính, gọi trước 15 phút.",
  );
  ...
  formDateRangeVal = signal<DateRange>({
  ...
  formRichTextVal = signal<string>(
    "<h3>Báo cáo Quý III</h3><p>Hệ thống hoạt động <b>ổn định</b> 99.9% uptime.</p>",
  );
  ...
  readonly activeComponent = computed<ComponentDoc>(() => {
    return (
      this.allDocs.find((d) => d.id === this.selectedComponentId()) ||
      this.allDocs[0]
    );
  });
