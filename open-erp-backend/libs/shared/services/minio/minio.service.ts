...
export class MinioService implements IMinioService {
  ...
  async downloadStream(
    ...
    try {
      ...
      if (options?.start !== undefined || options?.end !== undefined) {
        ...
        // Specific version
        stream = await this.client.getObject(this.config.bucket, sanitizedKey, {
   ...
   */
  async getVersion(key: string, versionId: string): Promise<Readable> {
    return this.downloadStream(key, { versionId });
  }
  ...
  async copyObject(
    ...
    try {
      ...
      let copySource = `/${this.config.bucket}/${sanitizedSourceKey}`;
      if (options?.versionId) {
  ...
  async deleteObject(
    ...
    try {
      ...
      if (options?.softDelete) {
        ...
        await this.client.removeObject(this.config.bucket, sanitizedKey, {
          versionId: options?.versionId,
        } as any);
  ...
  async deleteObjects(
    ...
    try {
      ...
      if (options?.softDelete) {
        ...
        await this.client.removeObjects(this.config.bucket, sanitizedKeys);
  ...
  async objectExists(key: string): Promise<boolean> {
    try {
      const sanitizedKey = this.sanitizeKey(key);
      await this.client.statObject(this.config.bucket, sanitizedKey);
      return true;
    } catch (error: any) {
