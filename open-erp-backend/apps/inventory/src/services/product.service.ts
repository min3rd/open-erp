...
export class ProductService {
  ...
  async softDelete(id: string, userId: string) {
    ...
    // Cleanup media files from MinIO (soft delete by moving to trash)
    try {
      ...
      this.logger.error(`Error cleaning up media for product ${id}:`, error);
      // Continue with soft delete even if media cleanup fails
    }
  ...
  /**
   * Clean up old media when updating with new media
   */
  async cleanupReplacedMedia(
    productId: string,
    oldThumbnail?: any,
    oldMedia?: any[],
  ): Promise<void> {
