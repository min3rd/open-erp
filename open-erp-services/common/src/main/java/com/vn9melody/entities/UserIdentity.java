...
@Table(
    ...
    uniqueConstraints = {
        @UniqueConstraint(name = "uk_tenant_provider_user", columnNames = {"tenant_id", "provider", "provider_user_id"})
    },
    indexes = {
...
public class UserIdentity extends BaseTenantEntity {
     ...
     * - LDAP / Active Directory: Distinguished Name (DN) hoặc objectGUID/sAMAccountName
     * - SAML 2.0: NameID
     * - Local: username
     */
    ...
    /**
     * Dữ liệu JSON thô trả về từ IdP (chứa roles bên ngoài, LDAP groups, token attributes...)
     */
