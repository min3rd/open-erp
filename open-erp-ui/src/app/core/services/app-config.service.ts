...
const DEFAULT_CONFIG: AppConfig = {
  ...
  maintenance: {
    ...
    title: 'Hệ thống đang được nâng cấp bảo trì',
    message: 'Chúng tôi đang bảo trì để nâng cấp chất lượng dịch vụ.',
...
export class AppConfigService {
  ...
  async loadConfig(): Promise<AppConfig> {
    try {
      const response = await fetch('/config/app-config.json?v=' + Date.now());
      if (response.ok) {
        const data: AppConfig = await response.json();
        this.configSignal.set(data);
        console.log('[AppConfigService] Loaded runtime config:', data);
        return data;
      }
    } catch (err) {
      console.warn('[AppConfigService] Failed to fetch /config/app-config.json, using default config:', err);
    }
