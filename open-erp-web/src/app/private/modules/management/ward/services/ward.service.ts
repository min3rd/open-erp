...
import { Ward, GetWardsParams, CreateWardDto, UpdateWardDto, ImportResult } from '../ward.types';
...
export class WardService {
  ...
  getWards(
  ...
  ): Observable<{ items: Ward[]; total: number; page: number; limit: number; totalPages: number }> {
    ...
    return this.http
      .get<ApiPaginatedResponse<Ward>>(`${API_URI_COMMON}/v1/wards`, {
        params: httpParams,
      })
      .pipe(
        map((response) => {
  ...
  getWard(code: string): Observable<Ward> {
    return this.http.get<ApiSingleResponse<Ward>>(`${API_URI_COMMON}/v1/wards/${code}`).pipe(
      map((response) => {
        const data = unwrap(response);
        return data.item!;
      }),
  ...
  createWard(dto: CreateWardDto): Observable<Ward> {
    return this.http.post<ApiSingleResponse<Ward>>(`${API_URI_COMMON}/v1/wards`, dto).pipe(
      map((response) => {
        const data = unwrap(response);
        return data.item!;
      }),
  ...
  updateWard(code: string, dto: UpdateWardDto): Observable<Ward> {
    return this.http.patch<ApiSingleResponse<Ward>>(`${API_URI_COMMON}/v1/wards/${code}`, dto).pipe(
      map((response) => {
        const data = unwrap(response);
        return data.item!;
      }),
  ...
  importWards(file: File): Observable<ImportResult> {
    ...
    return this.http
      .post<ApiSingleResponse<ImportResult>>(`${API_URI_COMMON}/v1/wards/import`, formData)
      .pipe(
        map((response) => {
          const data = unwrap(response);
          return data.item!;
