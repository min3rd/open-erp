...
@Component({
  ...
  templateUrl: './wms-nav.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WmsNav implements OnInit, OnDestroy {
  ...
  private updateActiveStates(): void {
    this.items = this.items.map((item) => this.updateItemActiveState(item));
  }

  private updateItemActiveState(item: MenuItem): MenuItem {
    ...
    if (item.items?.length) {
      updatedItem.items = item.items.map((child) => this.updateItemActiveState(child));
    }
    return updatedItem;
  }
