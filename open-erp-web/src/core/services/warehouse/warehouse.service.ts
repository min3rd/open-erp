...
/**
 * Minimal DTO for quick-create warehouse flow.
 * `code` is auto-generated by the server when omitted.
 */
export interface QuickCreateWarehouseDto {
  name: string;
  code?: string;
  addressDetail?: string;
  tenantId?: string;
...
export class WarehouseService {
  ...
  /**
   * Quick-create a warehouse with minimal required fields.
   * The server auto-generates `code` and defaults `type` to GENERAL when omitted.
   * POST /warehouses
   */
  quickCreateWarehouse(dto: QuickCreateWarehouseDto): Observable<Warehouse> {
    return this.http
      .post<ApiResponse<{ item: Warehouse }>>(this.baseUrl, dto)
      .pipe(map((response) => response.data?.item!));
  }
